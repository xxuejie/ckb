...
/// The `DatabaseStore` trait provides chain data store interface
pub trait DatabaseStore: Send + Sync + Sized {
...
/// A helper macro to implement ChainStore for a DatabaseStore struct
#[macro_export]
macro_rules! impl_chain_store_for_db_store {
    (@methods) => {
    ...
    ($type:ty) => {
        const _: () = {
            use ckb_db_schema::{
                ...
                COLUMN_UNCLES, META_CURRENT_EPOCH_KEY, META_LATEST_BUILT_FILTER_DATA_KEY,
                META_TIP_HEADER_KEY,
            };
            use ckb_db::iter::Direction;
            use ckb_types::{
                bytes::Bytes,
                core::{
            ...
            impl ckb_traits::ChainStore for $type {
                $crate::impl_chain_store_for_db_store!(@methods);
            }
        };
    };
    ($type:ty, < $( $gens:tt ),+ >) => {
        const _: () = {
            ...
            impl < $( $gens ),+ > ckb_traits::ChainStore for $type {
                $crate::impl_chain_store_for_db_store!(@methods);
            }
        };
    };
